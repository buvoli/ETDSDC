...
% RETURNS
%   ys      - solution values ys(:,i) solution at time t(i)
...
tic;
for i=1:Nt
    Nv = N(t0,y0,params,false);
    a  = E2.*y0 + 0.5*A.*Nv;
    Na = N(t0+h/2,a,params,false);
    b  = E2.*y0 + 0.5*A.*Na;
    Nb = N(t0+h/2,b,params,false);
    c  = E2.*a + A.*(Nb - 0.5*Nv);
    Nc = N(t0+h,c,params,false);
...
%INITETDRK4 Initializes coefficients for ETDRK4 for scaler/vector Lambda
    E = exp(h*Lambda);
    P = phi(h/2*Lambda,1);
        E2  = P(1,:);
        A   = h*P(2,:);   
